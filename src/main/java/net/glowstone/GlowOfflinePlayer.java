...
public final class GlowOfflinePlayer implements OfflinePlayer {
    ...
    public static CompletableFuture<GlowOfflinePlayer> getOfflinePlayer(
            GlowServer server, UUID uuid) {
        checkNotNull(server, "server must not be null");
        checkNotNull(uuid, "UUID must not be null");
        return ProfileCache.getProfile(uuid).thenApplyAsync(
            (profile) -> new GlowOfflinePlayer(server, profile));
    }

    /**
     * Deserializes an offline player as specified in
     * {@link org.bukkit.configuration.serialization.ConfigurationSerializable}.
     * @param val an offline player in serial form
     * @return {@code val} as an OfflinePlayer instance
     */
