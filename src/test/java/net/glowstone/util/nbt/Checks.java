...
class Checks {
    ...
    static void checkBigTest(CompoundTag compound) {
        ...
        assertThat("floatTest", compound.getFloat("floatTest"), IsFloatCloseTo.closeTo(0.49823147058486938f, 1e-16f));
        assertThat("doubleTest", compound.getDouble("doubleTest"), IsCloseTo.closeTo(0.49312871321823148, 1e-16));
        assertThat("stringTest", compound.getString("stringTest"), is("HELLO WORLD THIS IS A TEST STRING \u00c5\u00c4\u00d6!"));

        // byte array
        byte[] array = compound.getByteArray(BYTE_ARRAY_NAME);
        assertThat("byteArray size", array.length, is(1000));
        for (int i = 0; i < 1000; ++i) {
            assertThat("byteArrayTest[" + i + "]", array[i], is((byte) ((i * i * 255 + i * 7) % 100)));
        }
        ...
        assertThat("nested.ham.value", ham.getFloat("value"), IsFloatCloseTo.closeTo(0.75f, 1e-10f));
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            assertThat("compoundList[" + i + "].created-on", child.getLong("created-on"), is(1264099775885L));
            assertThat("compoundList[" + i + "].name", child.getString("name"), is("Compound tag #" + i));
        }
    }
