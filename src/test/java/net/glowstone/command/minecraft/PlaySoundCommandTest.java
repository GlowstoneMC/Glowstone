...
public class PlaySoundCommandTest {
    ...
    public void testExecuteFailsWithIncorrectSound() {
        assertThat(command.execute(opSender, "label", new String[]{"noise", "source", "player"}), is(false));
    ...
    public void testExecuteFailsWithIncorrectSoundCategory() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "source", "player"}), is(false));
        Mockito.verify(opSender).sendMessage(eq(ChatColor.RED + "'source' is not a valid sound category."));
    }

    @Test
    public void testExecuteFailsUnknownTarget() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player"}), is(false));
    ...
    public void testExecuteFailsMinimumVolumeOutOfRange() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player1", "0", "0", "0", "300", "2", "300"}), is(false));
        Mockito.verify(opSender).sendMessage(eq(ChatColor.RED + "Minimum volume value (300) must be between 0 and 1"));
    }

    @Test
    public void testExecuteFailsMinimumVolumeInvalid() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player1", "0", "0", "0", "300", "2", "volume"}), is(false));
    ...
    public void testExecuteFailsPitchOutOfRange() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player1", "0", "0", "0", "300", "300", "1"}), is(false));
        Mockito.verify(opSender).sendMessage(eq(ChatColor.RED + "Pitch value (300) must be between 0 and 2"));
    }

    @Test
    public void testExecuteFailsPitchInvalid() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player1", "0", "0", "0", "300", "pitch", "1"}), is(false));
    ...
    public void testExecuteFailsVolumeInvalid() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player1", "0", "0", "0", "volume", "1", "1"}), is(false));
        Mockito.verify(opSender).sendMessage(eq(ChatColor.RED + "'volume' is not a valid number"));
    }

    @Test
    public void testExecuteFailsPositionInvalid() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player1", "0", "0", "Y", "30", "1", "1"}), is(false));
    ...
    public void testExecuteFailsPlayerTooFarAway() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player1", "100000", "100000", "100000"}), is(false));
        Mockito.verify(opSender).sendMessage(eq(ChatColor.RED + "player1 is too far away to hear the sound"));
    }
    ...
    public void testExecuteSucceedsOnAnotherPlayerWithMinimumVolume() {
        assertThat(command.execute(opSender, "label", new String[]{"minecraft:entity.parrot.imitate.wither", "master", "player1", "0", "0", "0", "10", "0.6", "1"}), is(true));
        Mockito.verify(Bukkit.getPlayerExact("player1")).playSound(any(Location.class), eq(Sound.ENTITY_PARROT_IMITATE_WITHER), eq(SoundCategory.MASTER), eq((float) 1), eq((float) 0.6));
    }
    ...
    public void testTabComplete() {
        ...
        assertThat(command.tabComplete(opSender, "alias", new String[]{"sound", "hostile", ""}), is(ImmutableList.of("player1", "player2", "thePlayer3")));
        assertThat(command.tabComplete(opSender, "alias", new String[]{"sound", "hostile", "play"}), is(ImmutableList.of("player1", "player2")));
        assertThat(command.tabComplete(opSender, "alias", new String[]{"sound", "hostile", "the"}), is(ImmutableList.of("thePlayer3")));

        assertThat(command.tabComplete(opSender, "alias", new String[4]), is(Collections.emptyList()));
        assertThat(command.tabComplete(opSender, "alias", new String[5]), is(Collections.emptyList()));
        assertThat(command.tabComplete(opSender, "alias", new String[6]), is(Collections.emptyList()));
        assertThat(command.tabComplete(opSender, "alias", new String[7]), is(Collections.emptyList()));
        assertThat(command.tabComplete(opSender, "alias", new String[8]), is(Collections.emptyList()));
        assertThat(command.tabComplete(opSender, "alias", new String[9]), is(Collections.emptyList()));
        assertThat(command.tabComplete(opSender, "alias", new String[10]), is(Collections.emptyList()));
    }
}
